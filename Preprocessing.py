...
def ball_embed(ball_img, bg_img):
    ...
    ball_img = cv2.erode(mask,kernel,iterations = 1)
    
    #Scale and rotate the ball by random amounts
    rescale = np.random.random
    rotate = 360*np.random.random
    ball_img = cv2.resize(ball_img, fx=rescale, fy=rescale, interpolation = cv2.INTER_CUBIC)
    M = cv2.getRotationMatrix2D((cols/2,rows/2),rotate,1)
    ball_img = cv2.warpAffine(ball_img,M,(128,128))
    
    #Embed ball randomly into background
    w,h,_ = bg_img.shape
    offsetY = np.random.random_integers(0, h)
    offsetX = np.random.random_integers(0, w)
    bg_img[offsetY:offsetY+h,offsetX:offsetX+w] = ball_img
    
    mask = mask_Maker(bg_img, offsetY, offsetX)
    
    bg_img = cv2.resize(bg_img, (128, 128), interpolation = cv2.INTER_CUBIC)
    
    return bg_img, mask

def mask_Maker(bg_img, offsetY, offsetX):
    blank = np.zeros(bg_img.shape, dtype=int)
    blank[offsetY:offsetY+h,offsetX:offsetX+w] = ball_img
    ball_mask = cv2.threshold(blank,1,255,cv2.THRESH_BINARY)
